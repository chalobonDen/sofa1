...
$userQuery = "SELECT * FROM `invoice_from_vendors` JOIN document_header_purchase USING(Doc_purchase_ID)
                JOIN `line_item_purchase` ON `line_item_purchase`.Doc_purchase_ID = `document_header_purchase`.Ref_ID
                JOIN materials USING(Material_ID)
                WHERE `invoice_from_vendors`.`status` = 1 AND document_header_purchase.`Doc_type_ID` = 2 
...
<center>
	<div><br><br><br><br>
    <h1><b>Invoice form Vendor List</b></h1><br><br>
</div> 
    ...
    <tbody>
		<?php while ($row = mysqli_fetch_assoc($result)) { ?>
			<tr>
                ...
                <td><?=$row['Invoice_ven_ID'];?></td>
				<td><?=$row['Material_ID'];?></td>
				<td><?=$row['Material_name'];?></td>
                <td><?=$row['Invoice_date'];?></td>
		<?php } ?>
				
					<form method="post" action="select_inv_ven.php">
                        <input type="hidden" name="Doc_purchase_ID" value="<?=$row['Doc_purchase_ID'];?>">
						<input type="hidden" name="Invoice_ven_ID" value="<?=$row['Invoice_ven_ID'];?>">
						<input type="hidden" name="Material_ID" value="<?=$row['Material_ID'];?>">
			        	<input type="hidden" name="Material_name" value="<?=$row['Material_name'];?>">
                        <input type="hidden" name="Invoice_date" value="<?=$row['Invoice_date'];?>">

                    	<input type="submit" name="goodreceive" value="View Invoice" class="genric-btn danger radius">
					</form><br>
					<br>				
			</tr>
			<br>
